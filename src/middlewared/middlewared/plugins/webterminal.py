...
in main.py. See the internal development record for the full design record.
...
Three concerns kept deliberately separate, per the epic:
  ...
  2. Execution — a C-level fork()+execve() (`subprocess.Popen`, i.e.
     `_posixsubprocess`: nothing but async-signal-safe calls between fork
     and exec, no Python in the child) of a tiny single-purpose bootstrap
     that itself calls `setsid()` + acquires the pty as its controlling
     terminal (`TIOCSCTTY`) + `execve()`s the real command. The DoS in
     internal development note on the epic came from `os.forkpty()` running
     *Python* in a forked child of this multi-threaded daemon; that window
     does not exist here. Phase 1 used `os.posix_spawn` instead, and that
     had to go: FreeBSD 15 libc runs the posix_spawn child on a 4 KiB
     malloc'd stack inside this daemon's address space, and the child's
     first lazily-bound libsys call (execve) overflows it into our heap —
     the internal development record / the internal development record (the codeberg
     freecore/freecore#11 core dump). The bootstrap still does the
     session/ctty dance itself as a fresh, single-threaded process.
...
class WebTerminalService(ConfigService):
    ...
    async def get_ticket(self, app, data):
        ...
        # behaviour change guard 1 of the internal development record forbids.
...
# Runs as a freshly execve()'d, single-threaded process. The parent side is
# _posixsubprocess' fork()+execve() with no Python in between, so there is
# nothing frozen to inherit. Does explicitly what forkpty() used to do in the
# child branch; the session/ctty setup happens here, after exec, on purpose
# (subprocess's start_new_session would make the child a session leader
# before this setsid() and still leave the ctty unacquired).
_BOOTSTRAP = (
...
def _spawn(command):
    ...
    # (spawned /bin/sh AND /usr/bin/login both got a same-instant EOF
    ...
    # fork()+execve() through _posixsubprocess: between fork and exec the
    # child runs only async-signal-safe C (no Python, no locks) — the same
    # mechanism every subprocess.run() in this daemon already uses from
    # worker threads. Deliberately not os.posix_spawn(): FreeBSD 15 libc runs
    # the posix_spawn child on a 4 KiB malloc'd stack in *our* address space
    # (rfork(RFSPAWN)), and the child's first lazily-bound libsys call
    # (execve) overflows it into this daemon's heap. See
    # the internal development record and the internal development record.
    proc = subprocess.Popen(argv, env={
...
class ShellApplication2:
    ...
    async def ws_handler(self, request):
        ...
        proc, master_fd, slave_fd = _spawn(command)
        pid = proc.pid
        ...
        try:
            ...
            await self._reap(proc, t_session)
    ...
    async def _reap(self, proc, t_session):
        pid = proc.pid
        ...
        # take it out of the process table. These are children of
        ...
        for _ in range(3):
            # Popen.poll() is waitpid(WNOHANG) plus the object's own
            # bookkeeping: a reaped child is never re-queued for a later
            # Popen._cleanup() pass, and ECHILD is folded into a returncode.
            if await self.middleware.run_in_thread(proc.poll) is not None:
                break
