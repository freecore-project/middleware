...
class SystemRollbackService(Service):
    ...
    async def capture(self, arrival_path, captured_epoch=None):
        ...
        on a data pool and is unreachable until the
    ...
    async def cleanup(self):
        ...
        # freecore/the internal development record: find out whether there is anything left to do before
        # touching or logging anything.  Once the keep pin is released and every recorded
        # snapshot is gone, the hourly pass must be a silent no-op; re-recording the
        # closure every hour made the log read as if the window kept closing.
        origin_pinned = True
        try:
            origin = await self.middleware.call('bootenv.query', [['id', '=', window['origin_be']]])
            origin_pinned = bool(origin and origin[0].get('keep') is True)
        except Exception:
            self.logger.warning(
                'Failed to query boot environment %r for the rollback cleanup', window['origin_be'], exc_info=True
            )

        if not origin_pinned and not window['snapshots']:
            return window
