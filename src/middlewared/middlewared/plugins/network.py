...
async def ntpd_interface_post_sync(middleware):
    # freecore/the internal development record: FreeBSD 15's ntpd (4.2.8p18) never recovers from an
    ...
    #
    # freecore/the internal development record: never do this while the system is still booting.
...
async def _ntpd_system_ready(middleware, event_type, args):
    ...
    middleware.logger.info(
        'ntpd is not synchronised %d s after system ready; restarting it (freecore/the internal development record)',
        NTPD_READY_GRACE,
    )
...
async def setup(middleware):
    ...
    # freecore/the internal development record: restart ntpd once the network is fully settled if
