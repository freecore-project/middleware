...
def _note_html_response(verb, testpath, response):
    ...
    defect took a night to find (the internal development record -- service.start cifs never
...
def _ssh_failure(process):
    ...
    # rc==255 with no ssh transport marker. ssh reserves 255 for its own
    # failures but ALSO passes through a remote command that itself exits
    # 255 -- pdbedit(8), for one, prints "Username not found!" and exits 255,
    # which is exactly what test_275_ldap's "not listed" cases assert. A
    # genuinely dead target instead yields a transport marker (handled
    # above), so if the remote produced any output the command ran: report
    # its status as a result, not a dead target. Train-agnostic: 13.3 and
    # 15.x pdbedit behave identically. See the internal development record.
    _warn = re.compile(r'^Warning: Permanently added .*$', re.M)
    residual = _warn.sub('', err).strip()
    if (process.stdout or '').strip() or residual:
        return None
    # Nothing came back at all -- conservative reading is unreachable.
    return SSHConnectionError
